         ...
         ?>
                    ...
                    <ul id="tree" class="treeview">
                        <?php
                            ...
                            for ($i = 0; $i< $tam ; $i++){
                                ...
                                $num_logs = count($documentos[$i]['logs']);
                                //$previstoT = sum_the_time($previstoT, $documentos[$i]['previsto']);
                                $num = 1;
                                $filhos = "";
                                $duracao_filhos = "00:00:00";
                                foreach ($documentos[$i]['logs']  as $doc) {

                                    //$duracao_subtela = ProTemplate::model()->getTempoSubTela($doc['usuario'], $doc['data'], $doc['id']);
                                    $tempoSub = 0;// $duracao_subtela[0]['duracao'];
                                    $duracao_documento = time_to_seconds($doc['duracao'])+ $tempoSub;
                                    $duracao_formatado = sec_to_time($duracao_documento);

                                    $duracao_filhos = sum_the_time($duracao_filhos, $duracao_formatado);
                                    if ($num = $num_logs) {
                                        $filhos .= '<li>' . $doc['descricao'] . ' - '.$doc['nome'].' <a href="#">' . $duracao_formatado . '</a> '.Helper::dateDB2View($doc['data']).'</li>';
                                    } else {
                                       $filhos .= '<li class="last">' . $doc['descricao']  . ' - '.$doc['nome'].' <a href="#">' . $duracao_formatado . '</a> '.Helper::dateDB2View($doc['data']).'</li>';
                                    }
                                }
			...
			//d($i);
                               /* if ($duracao_filhos == "00:00:00"){
                                ...
                                '. $documentos[$i]['documento'] . ' - <a href="#"> </a>' . ' Realizado: '.$duracao_filhos. ' Documentos (' .$num_logs. ') ';
                              ...
                              //echo CHtml::link('Deletar',Yii::app()->getBaseUrl(true).'/documento/deletar/'.$documentos[$i]['documento_id'],array('confirm' => 'Deseja realmente excluir este documento?'));
                                ...
                                /* if(!$documentos[$i]['finalizado'] && (Yii::app()->user->groupName == "coordenador"||Yii::app()->user->groupName == "empresa")) {
                                     ...
                                     array('proDocumento/close/'.$documentos[$i]['documento_id']), // the URL for the AJAX request. If empty, it is assumed to be the current URL.
                                     array(
