 ...
 * @property string $ban
 * @property string $last_change_passwd
 *
...
class UserGroupsUser extends CActiveRecord {
    ...
    public function rules() {
        ...
        $rules = array(
            ...
            array('email', 'unique','on'=>array('insert'), 'message'=>Yii::t('smith','Este email já está sendo utilizado. Por favor utilize outro email.')),
            array('username', 'unique','on'=>array('insert'), 'message'=>Yii::t('smith','Este login já está sendo utilizado. Por favor escolha  outro login de acesso.')),
            ...
            // array('password', 'ext.validators.EPasswordStrength', 'on' => array('update')),
    ...
    public function relations() {
        Yii::import('application.modules.userGroups.models.UserGroupsGroup');
        Yii::import('application.modules.userGroups.models.UserGroupsAccess');
    ...
    public function attributeLabels() {
        return array(
            ...
            'image' => Yii::t('userGroupsModule.general', 'Logo da Empresa'),
    ...
    public function search() {
        ...
        if ($criteria->condition == null) {
            ...
            $criteria->condition = " t.serial_empresa like '{$serial}' AND email NOT LIKE '%admin%'";
        } else {
            $serial = MetodosGerais::getEmpresaId();
            $criteria->condition .= " AND t.serial_empresa like '{$serial}' AND email NOT LIKE '%admin%'";
        }
        if(MetodosGerais::getEmpresaId() == 41)
            $criteria->condition .= " AND t.fk_empresa != 41";
    ...
    protected function beforeSave() {
        if (parent::beforeSave()) {
            Yii::import('ext.validators.EPasswordStrength');

            // set the new user creation_date
            if ($this->isNewRecord)
                $this->creation_date = date('Y-m-d H:i:s');
            else
                $senha = UserGroupsUser::model()->findByPk($this->id)->password;
            ...
            if (($this->isNewRecord || $this->scenario === 'recovery' || $this->scenario === 'changePassword') && !empty($this->password)) {
                if (EPasswordStrength::checkPasswordStrength($this->password))
                    $this->password = md5($this->password . $this->getSalt());
                else {
                    return false;
                }
            }
            // Aplica o md5 ao atualizar senha no perfil do usuário
            if (($this->scenario === 'update') && !empty($this->password) && $this->password != $senha) {
                if (EPasswordStrength::checkPasswordStrength($this->password))
                    $this->password = md5($this->password . $this->getSalt());
                else {
                    return false;
...
//			$salt .= Yii::app()->controller->module->salt;
    ...
    public function checkTempoPasswd() {
        $noventaDias = date('Y-m-d', mktime(0, 0, 0, date("m"), date("d") - 90, date("Y")));
        return ((strtotime($this->_identity->getLastChangePasswd()) - strtotime($noventaDias)) <= 0) ? true : false;
    }

    /**
     * Verifica se desde o dia da implantação até o momento
     * já se passaram os dias que foi enviado por parâmetro
     * @param int $days
     * @return boolean
     */
    public function implantationIsAfterDays($days) {
        $afterDays = strtotime("+$days days", strtotime($this->creation_date));
        $today = strtotime(date('Y-m-d H:i:s'));

        if ($this->group_id == 3 && $afterDays < $today)
            return true;
        return false;
    }

    public function getUsuarioEmpresa(){
        return $this->username . ' - ' . Empresa::model()->findByPk($this->fk_empresa)->nome;
    }
