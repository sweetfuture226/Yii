...
class WebUserGroups extends CWebUser
{
	 ...
	 * @param string $serial_empresa
         * @param int $equipe
	 *
	 * @see CWebUser::changeIdentity()
	 */
	protected function UGChangeIdentity($id, $name, $groups, $groupName, $level, $accessRules, $home, $recovery, $profile, $states, $fk_empresa, $serial_empresa, $equipe)
	{
	    $this->setId($id);
	    $this->setName($name);
		...
		$this->setEmpresaId($fk_empresa);
            $this->setSerialEmpresa($serial_empresa);
	...
	public function login($identity,$duration=0)
	{
	    ...
	    if($this->beforeLogin($id,$states,false))
	    {
	        $this->UGChangeIdentity($id,$identity->getName(), $identity->getGroup(), $identity->getGroupName(), $identity->getLevel(),
	        	$identity->getAccessRules(), $identity->getHome(), $identity->getRecovery(), $identity->getProfile(), $states, $identity->getEmpresa_id(), $identity->getSerial_empresa(), $identity->getFk_equipe());
	...
	public function getProfile()
	{
        ...
        {
			$this->setState("__fk_empresa", $value);
        }

        public function getEmpresaId()
        {
			return $this->getState("__fk_empresa");
        }

	public function setSerialEmpresa($value)
        {
            $this->setState("__serial_empresa", $value);
        }

	public function getSerialEmpresa()
        {
            return $this->getState("__serial_empresa");
        }

	public function setEquipe($value)
        {
            $this->setState("__equipe", $value);
        }

	public function getEquipe()
        {
            return $this->getState("__equipe");
        }
