...
function add_documento(salvar) {
    ...
    $('#status_documento').removeClass('entypo-check');
    $('#status_documento').addClass('entypo-block');
...
$('#disciplina_nome').on('keyup', function() {
    delay(function () {
        $.ajax({
            ...
            $('#status_disciplina').removeClass('entypo-check');
            $('#status_disciplina').addClass('entypo-block');
            $('#status_disciplina').attr({style: 'font-size: 1.5em; line-height: 40px; color: #ff6c60'});
            $('#submit_dis').attr({disabled: 'disabled'});
        }).fail(function() {
            $('#status_disciplina').removeClass('entypo-block');
            $('#status_disciplina').addClass('entypo-check');
    ...
    if (contrato == 'create') {
        delay(function () {
            if ($.inArray($.trim($('#Documento_nome').val()), getValues()) > -1 || $('#Documento_nome').val() == '') {
                ...
                $('#status_documento').removeClass('entypo-block');
                $('#status_documento').addClass('entypo-check');
