$(function() {
    ...
    var valor = $("#ContaReceber_valor_bruto");
    var vencimento = $("#ContaReceber_data_vencimento");
    ...
    $("#conta-receber-form").submit(function(){
       ...
       if(qnt_parcelas > 1){
            ...
            var p;
            var total = string2float($("input[name='ContaReceber[valor_bruto]']").val());
            var v = parseFloat('0');
            ...
            for(i=0; i<qnt_parcelas; i++){
                p = parseFloat(string2float($("input[name^='ContaReceber[parcelados]["+i+"][valor_bruto]']").val())).toFixed(2);
                v = v + parseFloat(p);
            ...
            var template = '\
                    ...
                    <div class="form-group  col-lg-12" style="padding: 0">\n\
                        <div class="form-group  col-lg-2">\n\
                            <label for="ContaReceber_valor_bruto_'+index+'">Valor da  '+a+'ª parcela</label>\n\
                            <input class="dinheiro form-control" name="ContaReceber[parcelados]['+index+'][valor_bruto]" id="ContaReceber_valor_bruto_'+index+'" class="valor text" type="text" value="'+valorParcela+'">\n\
                        </div>\n\
                        <div class="form-group  col-lg-2">\n\
                            <label for="ContaReceber_'+index+'_data_vencimento">Data de vencimento</label>\n\
                            <input class="date form-control" name="ContaReceber[parcelados]['+index+'][data_vencimento]" id="ContaReceber_'+index+'_data_vencimento" type="text" size="10" '+evento+'>\n\
                        </div>\n\
                    </div>\n\
                    ';
...
function changeDates(data)
{
    ...
    if(data != '')
    {
        ...
        for(i=1; i<qtd;i++)
        {
            ...
            document.getElementById('ContaReceber_'+i+'_data_vencimento').value = nova_data;
        }
    }
}
...
function parcelapercent(id,valor_parcela) {
    ...
    var valor_total = document.getElementById("ContaReceber_valor_bruto").value;
...
function parcelavalor(id,valor_percentual)
{
    ...
    if(percent_aux == 100)
    {
        for(i=0; i < qtd;i++)
        {
            if(i != id_percentual)
            {
                var aux = string2float(document.getElementById('ContaReceber_valor_bruto_'+i).value);
    ...
    document.getElementById('ContaReceber_valor_bruto_'+id_percentual).value = valor_parcela;
