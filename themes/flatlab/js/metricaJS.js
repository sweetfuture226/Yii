...
function validaForm() {
    ...
    if ($(".membros").val() == null) {
        document.getElementById('message').innerHTML = "Selecione a equipe e os colaboradores que utilizam esta métrica.";
        $('#btn_modal_open').click();
        valido = false;
    }
    ...
    if ($('#Metrica_id').val() != null) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#colaboradores_selecionados').html('<li class="search-field" style="width: 100%;"><input type="text" value="" class="default" autocomplete="off" style="width: 100%;"></li>');
                $('#colaboradores_selecionados').prepend(data);
...
$(function () {
    ...
    $("#slider-range-mini").slider({
        ...
        max: 1000,
        slide: function (event, ui) {
            $("input.min").val(ui.value);
        }
    });
    $("#slider-range-maxi").slider({
        range: "min",
        value: $("input.max").val(),
        min: 1,
        max: 1000,
        slide: function (event, ui) {
            $("input.max").val(ui.value);
        }
    });
});
...
$("#Metrica_min_e").on('change', function () {
    $('#slider-range-mini').slider({value: this.value});
});
$("#Metrica_max_e").on('change', function () {
    $('#slider-range-maxi').slider({value: this.value});
});
